...
struct statistika {
    ...
    std::chrono::time_point<std::chrono::high_resolution_clock> vrijemeIducePojave;
    zadatak zad;
};
...
void upravljackaPetljaKS(vector<int> brojNeodradjenih, vector<statistika> neStigneOdraditi, auto pocetnoVrijeme){

    while(simulacijaTraje){

        if (neStigneOdraditi.size() > 0){

            statistika zad;
            zad = neStigneOdraditi.front();
            neStigneOdraditi.erase(neStigneOdraditi.begin());
            ...
            {
                lock_guard<mutex> lock(mtx);
                outputFile << vrijemePocetkaMs << "\tUprKS: započinjem obradu ulaza-" << zad.zad.idUlaza << "\n";
            }
            ...
            brojNeodradjenih[zad.zad.idUlaza - 1] = 0;
            ...
            {
                lock_guard<mutex> lock(mtx);
                outputFile << vrijemeKrajaMs << "\tUprKS: gotova obrada ulaza-" << zad.zad.idUlaza << "\n";
            }


        }
        else {
            //while(std::find(brojNeodradjenih.begin(),brojNeodradjenih.end, 2))
            break;
        }
    }
}

void upravljackaPetlja(int zadNum){
    //std::cout << "upravljacka petlja" << std::endl;

    auto pocetnoVrijeme = std::chrono::high_resolution_clock::now();

    vector<int> brojNeodradjenih(zadNum, 0);

    while(simulacijaTraje){

        vector<statistika> neStigneOdraditi;

        if (redCekanja.size() > 0){

            statistika zad;

            bool traziZadatak = true;
            ...
            while(traziZadatak){
                ...
                if (zad.vrijemeIducePojave > std::chrono::high_resolution_clock::now() + std::chrono::milliseconds(zad.zad.period)){
                    traziZadatak = false;
                } else {
                    neStigneOdraditi.push_back(zad);
                    brojNeodradjenih[zad.zad.idUlaza - 1] += 1;
                    if (brojNeodradjenih[zad.zad.idUlaza - 1] >= 3){
                        upravljackaPetljaKS(brojNeodradjenih, neStigneOdraditi, pocetnoVrijeme);
                    }
                }
            }
            zad.vrijemePocetkaObrade = std::chrono::high_resolution_clock::now();
...
int main()
{
    ...
    threads.emplace_back(upravljackaPetlja, zadaci.size());
